...
const FRONTEND_IDENTITY = 'KAI_CLOUD_DUAL_WORKSPACE_PROVIDER_PUBLISH_V1';
...
function CloudPayApp() {
  ...
  useEffect(() => {
    if (!workModeReady || workMode !== 'provider' || activeTab !== 'workspace') return;
    void refresh();
  ...
  useEffect(() => {
    if (workMode !== 'provider' || activeTab !== 'workspace'
  ...
  useEffect(() => {
    void loadWorkMode().then((mode) => {
      setWorkMode(mode);
      setActiveTab(mode === 'provider' ? 'workspace' : 'home');
  ...
  const openResourceAction = useCallback((action: ProviderResourceAction) => {
    navigate('publish');
    if (action.target === 'resource') setOfferWizard({ resourceId: action.entityId });
    if (action.target === 'wizard_draft') setOfferWizard({ resumeDraftId: action.entityId });
    if (action.target === 'offer_revision') setOfferWizard({ revisionOfferId: action.entityId });
    if (action.target === 'offer_review') setPublishOfferToReveal(action.entityId);
    if (action.target === 'offer_listing') setListingOfferId(action.entityId);
    if (action.target === 'listing') setPublishListingToManage(action.entityId);
  }, [navigate]);

  const openProviderNextAction = useCallback((route: string, entityId: string | null) => {
  ...
  const page = (() => {
    switch (activeTab) {
      ...
      case 'profile':
        return <ProfileScreen snapshot={snapshot} onSessionChanged={refresh} onLogin={() => setAuthVisible(true)}
          onOpenPublish={() => changeWorkMode('provider')} onOpenCredits={() => setCreditWalletVisible(true)} />;
      case 'home':
      default:
        return <HomeScreen snapshot={snapshot} refreshing={refreshing} onRefresh={refresh} onNavigate={navigate} onOpenDemand={() => setDemandComposerVisible(true)} />;
    }
  })();
  ...
  return (
    <SafeAreaView nativeID={FRONTEND_IDENTITY} style={styles.safeArea} edges={['top', 'left', 'right']}>
      ...
      <WorkspaceHeader
        ...
        subject={snapshot.subjects.find((subject) => subject.id === snapshot.currentSubjectId) ?? null}
        subjects={snapshot.subjects}
        unread={snapshot.unreadCount}
        subjectPickerVisible={subjectPickerVisible}
        onOpenSubjectPicker={() => snapshot.authenticated ? setSubjectPickerVisible(true) : setAuthVisible(true)}
        onCloseSubjectPicker={() => setSubjectPickerVisible(false)}
        onSelectSubject={(subjectId) => void chooseSubject(subjectId)}
        onMessages={() => navigate('messages')}
        onProfile={() => navigate('profile')}
      />
      <View style={styles.page}>{page}</View>
      <BottomNav active={activeTab} mode={workMode} onChange={navigate} unread={snapshot.unreadCount} />
      <AuthSheet visible={authVisible} onClose={() => setAuthVisible(false)} onSignedIn={refreshAfterAuthentication} />
      <PublishFlowSheet
      ...
      <MarketOrderSheet
        ...
        onCreated={async (order) => { await refresh(); setSelectedOrder(order); }}
      />
