...
网页端新增“供应商返佣”板块。认证供应商可设置返佣比例、邀请 CloudPay 现有账号成为推广伙伴；伙伴接受邀请后获得专属链接。买方通过链接建立归因并完成订单验收后，返佣先进入等待期，待供应商结算可支付且无退款或纠纷时转为可支付。退款会自动撤销未支付返佣，已支付返佣会进入追偿状态。

默认规则为 8% 返佣、单笔最高 500 元、30 天归因期和 7 天等待期，均可通过 `KAI_SUPPLIER_REFERRAL_*` 环境变量调整。
