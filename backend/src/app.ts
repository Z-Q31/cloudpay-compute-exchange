...
export async function buildApp({ config, database, accountService, subjectService, marketService, notificationService, listingAuditService, operationsService, resourceEvidenceService, creditLedgerService, creditTopupService, creditOrderService, logger = true }: BuildAppOptions) {
  const app = Fastify({
    logger: logger ? {
      redact: {
        paths: [
  ...
  if (accountService) await registerAccountRoutes(app, accountService, config);
