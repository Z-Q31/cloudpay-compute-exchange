...
}> | Readonly<{ status: 'not_approved' }> | Readonly<{ status: 'audit_expired' }>;
export type SetListingStatusResult = Readonly<{ status: 'updated' | 'replayed'; listing: CreditListing }>
  | Readonly<{ status: 'not_found' | 'invalid_transition' | 'expired' | 'reserved_capacity' | 'approval_invalid' | 'capacity_unavailable' }>;
  ...
  async publishListing(input: Parameters<ListingAuditStore['publishListing']>[0]): Promise<PublishListingResult> {
    return this.database.transaction(async (client) => {
      ...
      if (replay.rows[0]) return replay.rows[0].payload_digest === input.payloadDigest
        ...
        : { status: 'conflict' };
      const current = await client.query<OfferRow & { capacity_total: string; resource_status: string; supplier_status: string }>(
        `SELECT o.${offerColumns.replaceAll(', ', ', o.')}, r.capacity_total::text, r.status AS resource_status,
