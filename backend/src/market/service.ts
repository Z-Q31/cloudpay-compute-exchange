...
export class MarketService {
  ...
  async createResource(
    ...
    const normalized = {
      ...
      specifications: publicSpecifications(input.specifications), capacityTotal: decimalQuantity(input.capacityTotal),
    ...
    const result = await this.store.createResource({
      id: randomUUID(), subjectId: subject.subjectId, requestedByUserId: principal.userId,
      ...normalized,
    ...
    return { replayed: result.status === 'replayed', recovered: result.status === 'existing', resource: { ...result.resource } };
  }
