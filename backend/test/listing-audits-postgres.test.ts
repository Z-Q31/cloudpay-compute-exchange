...
async function migrate(pglite: PGlite) {
  for (const name of ['0001_cloudpay_ledger.sql', '0003_market_reservations.sql', '0012_mobile_publish.sql', '0015_credit_listing_audits.sql', '0016_trading_subjects.sql', '0017_offer_wizard_drafts.sql', '0021_offer_revision_drafts.sql']) {
...
describe('credit listing double-audit workflow', () => {
  it('requires two reviewers, computes H100 credits exactly, and publishes only within audit validity', { timeout: 30_000 }, async () => {
    ...
    const alternate = await store.createOffer({
      ...
      title: 'H100 夜间算力', serviceMode: 'reserved',
    });
