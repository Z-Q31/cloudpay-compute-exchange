...
import type {
  ComputeDemand, ComputeResource, MarketListing, OrderRecord, ResourceKind, SupplierProfile, SupplierResource,
...
async function approvedListing(service: MarketService, store: MemoryMarketStore) {
  ...
  const resource = await service.createResource(supplier, {
    ...
    specifications: { gpuMemoryGb: 80, interconnect: 'NVLink' }, capacityTotal: '10', capacityUnit: 'GPU时', assetReference: 'ASSET-HARNESS-001', assetIdentityKind: 'hardware_serial',
...
describe('verified market and order lifecycle', () => {
  ...
  it('requires an actionable per-material decision when review sends a resource back', async () => {
    ...
    await service.reviewSupplier(operator, { supplierId: submitted.id, approved: true }, context);
    const resource = await service.createResource(supplier, {
      kind: 'gpu', productCode: 'H100-SXM-80G', region: '华东-上海', specifications: {},
  ...
  it('publishes verified resource facts without RMB pricing fields', async () => {
    ...
    await service.reviewSupplier(operator, { supplierId: submitted.id, approved: true }, context);
    const resource = await service.createResource(supplier, {
      kind: 'gpu', productCode: 'H100-SXM-80G', region: '华东-上海', specifications: { gpuMemoryGb: 80 },
  ...
  it('requires supplier approval and resource verification before publishing', async () => {
    ...
    await expect(service.createResource(supplier, {
      kind: 'gpu', productCode: 'H100', region: '上海', specifications: {}, capacityTotal: '10', capacityUnit: 'GPU时', assetReference: 'ASSET-UNAPPROVED-001', assetIdentityKind: 'hardware_serial',
  ...
  it('rejects credentials embedded in public resource specifications', async () => {
    ...
    await service.reviewSupplier(operator, { supplierId: submitted.id, approved: true }, context);
    await expect(service.createResource(supplier, {
      kind: 'gpu', productCode: 'H100', region: '上海', specifications: { sshPassword: 'never-public' }, capacityTotal: '10', capacityUnit: 'GPU时', assetReference: 'ASSET-SECRET-001', assetIdentityKind: 'hardware_serial',
  ...
  it('returns the existing resource when the same private asset identity is submitted again', async () => {
    ...
    await service.reviewSupplier(operator, { supplierId: submitted.id, approved: true }, context);
    const payload = {
      kind: 'gpu' as const, productCode: 'H100', region: '上海', specifications: {}, capacityTotal: '10',
