...
describe('mobile offer wizard drafts', () => {
  it('resumes safely, rejects stale saves, and submits one offer with both audits atomically', { timeout: 30_000 }, async () => {
    ...
    if (!created || created.status === 'conflict') throw new Error('draft was not created');
    ...
    if (!repeatedResource || repeatedResource.status === 'conflict') throw new Error('active draft was not reused');
    ...
    if (!disposable || disposable.status === 'conflict') throw new Error('disposable draft was not created');
    expect(await store.abandonWizardDraft({
