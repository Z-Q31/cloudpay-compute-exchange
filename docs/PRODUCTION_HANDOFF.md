...
Android 本地完整后端已完成提供方上架闭环验收，但生产环境尚未接管移动接口，因此当前不得提交商店。

正式预检当前有三类阻塞：

1. `cloudpay.kai.com` 的移动 API、法务页和账户删除页仍由旧网站返回 HTML；新的上架后端尚未切流。
2. `com.kaicloud.marketplace` 尚未绑定真实 Expo 项目 UUID，正式 AAB 无法配置推送。
3. 缺少当前主前端源码指纹的历史 APK/AAB 已移出项目；生产配置齐全后仍需重新生成正式候选物。

## 已交付

- 后端部署包：`artifacts/release/KAI-CloudPay-backend-1.0.0-production.tar.gz`
- 后端 SHA-256：`0e5b2bedb059bf4bde6c3be67d924bc305705a729713f7945b934e66e1d4ca1f`
- 数据库迁移：35 份，最新为 `0035_offer_wizard_draft_abandonment.sql`
- 后端验证：39 个测试文件、163 项测试
- 部署包会在生成时从隔离目录执行生产依赖安装并独立加载编译入口；不能只依赖源码目录已有的开发环境。
- systemd 与容器两种入口都会在 API 启动前执行生产配置门禁；材料存储、安全扫描、推送、备份、法务或充值渠道不完整时直接拒绝启动。
- 目标机解压后只需运行 `npm run release:verify`；最新部署包已从全新临时目录实测 8 项全部通过，包括 251 个文件摘要、35 份迁移、生产依赖和两种失败关闭入口。
- 部署与路由契约作为打包门禁自动执行，当前 14 项全部通过：52 个环境变量均有来源，Kubernetes 明确覆盖 14 个公开设置和 47 个敏感设置；API、迁移和备份任务使用同一配置来源和同一不可变镜像摘要；ALB 与 Kubernetes 只接管手机版 API 和三个法务页；目标主机还必须通过 21 项失败关闭预检和十个关键路径的双入口旁路验收；失败报告只允许撤回这五条移动路由，旧站、旧 `/api/*` 和向前兼容迁移必须保留。
...
- 移动接口合同：92 个前端调用、81 条后端路由
- Android 本地验收包：`artifacts/release/KAI-CloudPay-1.0.0-1-local-e2e.apk`
- Android 本地验收包 SHA-256：`db016775bc9c6247dcf19c1caee8f158b1b7d7041f406800a4a17de8b17b4b83`
- 当前主前端源码指纹：`deac6f9726b5e9e5bf79192a625db341b036151398d79771d2392095770140ea`
- 当前主前端源码指纹覆盖 App 入口、页面、原生分发代码、插件、配置和锁定依赖；旧包无法通过 `frontend_source_current`。
- 设备安装门禁 27 项全部通过；同时核对候选包与设备内 APK 的 SHA-256，历史同包名应用不能冒充当前主包。
...
- 同一主包已实测完成方案双审、8 GPU时挂牌和购买视角市场可见，方案提交与挂牌连续点击均只生成一条服务端记录。
...
- 已实测挂牌暂停与恢复：暂停后购买市场从 1 条变为 0 条；恢复后重新显示同一挂牌，挂牌 ID、8 GPU时容量和 `31.137725` 卡时审核锁价均不变，没有重复发布。
...
- 当前回归结果：前端 38 项、后端 163 项、移动接口合同 92 个调用/81 条路由、Android 设备门禁 27 项全部通过。
...
- Android 已实测验收后 7 天售后：买方申请 20 卡时补偿后结算暂停；提供方待办显示 1、可同意或提出异议；平台同时看到买方说明、提供方依据和交付记录，裁决后买方到账 20、提供方剩余待结算 `42.27545`，已消耗容量不被恢复。
...
- 短信登录：`SMS_PROVIDER`、`SMS_ACCESS_KEY_ID`、`SMS_ACCESS_KEY_SECRET`、`SMS_SIGN_NAME`、`SMS_TEMPLATE_CODE`
...
1. 在隔离数据库运行 35 份迁移并通过 schema readiness。
...
5. ALB 只接管 `/mobile/v1`、`/mobile/v1/*`、`/privacy`、`/terms`、`/account/delete`；首页和 `/api/*` 保持原目标组。
