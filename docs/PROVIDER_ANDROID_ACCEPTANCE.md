...
本批安装包 SHA-256：`db016775bc9c6247dcf19c1caee8f158b1b7d7041f406800a4a17de8b17b4b83`

主前端源码指纹：`deac6f9726b5e9e5bf79192a625db341b036151398d79771d2392095770140ea`

## 验收结论

提供方已在 Android App 内完成一条不依赖旧前端的完整上架链路。每一步同时核对手机界面、移动端接口和服务端数据库状态；没有仅在客户端显示成功的步骤。

四步上架草稿已在真实 Android 操作中连续验证：人民币依据 `¥31.20` 按固定汇率换算为 `31.137725 KAI 卡时`，核价凭证与五项交付边界完整进入确认页；冷启动后服务端仍恢复到同一草稿的“提交确认”。补填会移除本地校验提示，但保存失败和版本冲突不会被掩盖。

## 本次完整路径

1. 同一账号切换到“提供算力”，冷启动后保持提供方视角。
2. 在“资源”准备并验真 `H100-E2E-OFFER`，提交 8 GPU时可用容量。
...
8. 发布 8 GPU时、立即生效、持续 7 天的挂牌；服务端生成 `active` 挂牌，App 显示“销售中”。

## 一致性检查

- 资源：手机显示 8 GPU时、已验真；服务端资源状态为 `verified`。
- 材料：手机三项均为“已完成”；服务端三份证据均为 `verified`。
- 方案：手机显示双审通过；服务端资源审、价格审均为 `approved`。
- 单价：页面建议价不直接成为挂牌价；挂牌使用审核锁定的 `31.137725 KAI 卡时 / GPU时`。
- 挂牌：手机显示 8 GPU时、销售中；服务端挂牌状态为 `active`，容量和 7 天时段一致。
...
- 挂牌闭环：本轮发布 8 GPU时、立即生效、持续 7 天；服务端状态为 `active`，购买视角立即出现 1 个挂牌，并识别为“我的挂牌”而不是购买按钮。
...
- 买卖闭环：双审通过后从 Android 发布 8 GPU时、立即生效、持续 7 天；切回“使用算力”后市场立即出现同一挂牌，并标记为“我的挂牌”。
...
设备门禁报告：`artifacts/release/android-device-smoke-report.json`，本轮 27 项全部通过，无 Metro 或致命运行错误。
