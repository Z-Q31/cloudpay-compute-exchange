...
const tabs = providerMode ? [
  ['home', ['提供工作台', '登录后上架算力', '正在同步上架数据', '上架数据没能载入']],
  ['market', ['我的资源']],
  ['publish', ['上架进度', '资源方入驻', '添加算力资源']],
  ['messages', ['消息']],
  ['profile', ['当前账户']],
] : [
  ['home', ['开始一次匹配']],
...
for (const [index, [id, markers]] of tabs.entries()) {
  ...
  const hierarchy = adbRun('exec-out', 'cat', remoteXml);
  ...
  check(`tab_${id}`, hasExpectedMarker && !hasRemovedPublishHeader, markers.join(' / '));
}
