...
if (!localE2e && !validProjectId(configuredProjectId)) {
  throw new Error('Production Android builds require CLOUDPAY_EAS_PROJECT_ID from the bound Expo project.');
}
if (!localE2e) {
  ...
  if (!binding.ok) throw new Error(`Production Android builds require a verified Expo binding. ${binding.evidence}`);
}
...
if (distributionChannel === 'direct-cn' && (!embeddedNativeTopups || !embeddedNewOrders)) {
...
if (!localE2e && (!validProjectId(embeddedProjectId) || embeddedProjectId !== configuredProjectId)) {
  process.stderr.write('Production artifact is missing the requested Expo push project ID.\n');
  process.exit(1);
}
...
if (!localE2e) {
  const forbiddenMarkers = ['10.0.2.2', '/__e2e/', 'CLOUDPAY_LOCAL_E2E'];
