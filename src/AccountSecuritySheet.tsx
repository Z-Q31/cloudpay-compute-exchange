      ...
      <KeyboardAvoidingView style={styles.backdrop} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.sheet}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            {loading ? (
              <View style={styles.loadingRow}><ActivityIndicator color={colors.green} /><Text style={styles.loadingText}>正在核对服务端状态…</Text></View>
            ) : null}
            {error ? (
              ...
              {sessions.length ? sessions.map((session, index) => (
                <View key={session.id} style={[styles.row, index === sessions.length - 1 && styles.rowLast]}>
                  <View style={styles.rowIcon}>
                    <Ionicons name={session.device.platform === 'android' ? 'logo-android' : 'phone-portrait-outline'} size={20} color={colors.greenDark} />
                  </View>
            ...
            <View style={styles.group}>
              <View style={styles.row}>
                ...
                {busyAction === 'push' ? <ActivityIndicator color={colors.green} /> : (
                  <Switch
                    value={pushEnabled}
              ...
              <View style={[styles.row, styles.rowLast]}>
                <View style={styles.rowIcon}>
                  <Ionicons name="mail-unread-outline" size={20} color={colors.greenDark} />
                </View>
                <View style={styles.rowCopy}>
              ...
              <View style={styles.deletionCard}>
                ...
                {['requested', 'cooling_off', 'blocked_by_legal_hold'].includes(deletion.status) ? (
                  <Pressable disabled={Boolean(busyAction)} onPress={cancelDeletion} style={styles.cancelDeletionButton}>
                    {busyAction === 'deletion-cancel' ? <ActivityIndicator color={colors.green} /> : <Text style={styles.cancelDeletionText}>撤回注销申请</Text>}
                  </Pressable>
                ) : null}
              </View>
            ) : (
...
const styles = StyleSheet.create({
  ...
  handle: { width: 42, height: 5, borderRadius: 3, alignSelf: 'center', backgroundColor: '#D4DDD7', marginBottom: 15 },
  ...
  rowIcon: { width: 38, height: 38, borderRadius: 13, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  ...
  cancelDeletionText: { color: colors.green, fontSize: 13, fontWeight: '800' },
  deletionForm: { padding: 15 },
  formHint: { color: colors.muted, fontSize: 12, lineHeight: 18, marginBottom: 8 },
  input: { minHeight: 48, paddingHorizontal: 14, marginTop: 9, borderWidth: 1, borderColor: colors.line, borderRadius: 14, color: colors.ink, backgroundColor: '#FAFCFB', fontSize: 14 },
