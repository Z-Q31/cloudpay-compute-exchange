        ...
        <View style={styles.sheet}>
          ...
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
            <View style={styles.amountCard}>
              ...
              <Text style={styles.amount}>{trim(review.order.totalCredits)} <Text style={styles.unit}>KAI 卡时</Text></Text>
            ...
            <View style={styles.remedyCard}><Text style={styles.remedyLabel}>申请补偿</Text><Text style={styles.remedyAmount}>{trim(review.creditAmount)} KAI 卡时</Text></View>
            ...
            <Text style={styles.notice}>{outcome === 'approve_refund'
              ? `将退回买方 ${trim(review.creditAmount)} KAI 卡时，剩余卡时按原日期结算。`
            ...
            <Pressable disabled={busy || reason.trim().length < 10} onPress={() => Alert.alert(
              ...
              outcome === 'approve_refund'
                ? `将退回 ${trim(review.creditAmount)} KAI 卡时；提交后不可撤回。`
...
function Choice({ active, label, onPress }: Readonly<{ active: boolean; label: string; onPress: () => void }>) {
  return <Pressable onPress={onPress} style={[styles.choice, active && styles.choiceActive]}><Ionicons name={active ? 'radio-button-on' : 'radio-button-off'} size={18} color={active ? colors.green : colors.subtle} /><Text style={[styles.choiceText, active && styles.choiceTextActive]}>{label}</Text></Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  eyebrow: { color: colors.green, fontSize: 9, fontWeight: '900', letterSpacing: 1.2 }, title: { color: colors.ink, fontSize: 21, fontWeight: '900', marginTop: 3 },
  ...
  amountCard: { padding: 18, borderRadius: 22, backgroundColor: '#0B4F2B' }, amountLabel: { color: '#B8F8CB', fontSize: 10, fontWeight: '900' },
  amount: { color: colors.surface, fontSize: 30, fontWeight: '900', marginTop: 7 }, unit: { fontSize: 12, color: '#C7EAD2' }, meta: { color: '#C7EAD2', fontSize: 9, marginTop: 6 },
  evidence: { padding: 15, marginTop: 12, borderWidth: 1, borderColor: colors.line, borderRadius: 18, backgroundColor: colors.surface }, label: { color: colors.green, fontSize: 10, fontWeight: '900' }, body: { color: colors.ink, fontSize: 12, lineHeight: 19, marginTop: 7 },
  remedyCard: { padding: 15, marginTop: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderRadius: 18, backgroundColor: colors.greenSoft }, remedyLabel: { color: colors.greenDark, fontSize: 11, fontWeight: '800' }, remedyAmount: { color: colors.greenDark, fontSize: 16, fontWeight: '900' },
  ...
  sectionTitle: { color: colors.ink, fontSize: 18, fontWeight: '900', marginTop: 19 }, choiceRow: { flexDirection: 'row', gap: 9, marginTop: 11 }, choice: { flex: 1, minHeight: 50, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', gap: 7, borderWidth: 1, borderColor: colors.line, borderRadius: 15, backgroundColor: colors.surface }, choiceActive: { borderColor: colors.green, backgroundColor: colors.greenSoft }, choiceText: { color: colors.muted, fontSize: 12, fontWeight: '800' }, choiceTextActive: { color: colors.greenDark },
  ...
  primary: { minHeight: 52, marginTop: 14, borderRadius: 17, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, primaryText: { color: colors.surface, fontSize: 13, fontWeight: '900' }, disabled: { opacity: 0.48 },
});
