...
const localE2E = Constants.expoConfig?.extra?.allowInsecureApiForLocalE2e === true;
...
export function AuthSheet({
  ...
  const requestCode = async () => {
    ...
    try {
      ...
      setResendAfter(challenge.resendAfterSeconds);
      if (localE2E) {
        try {
          const response = await fetch(`${CLOUDPAY_URL}/__e2e/otp`, { headers: { Accept: 'application/json' } });
          const localCode = localE2EOtpForPhone(phone, await response.json());
          if (localCode) { setCode(localCode); setLocalCodeFilled(true); }
          else {
            setLocalCodeFilled(false);
  ...
  const openDocument = async (url: string | undefined) => {
      ...
      <KeyboardAvoidingView style={styles.backdrop} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={styles.sheet}>
          ...
          <View style={styles.header}>
            <View>
              <Text style={styles.eyebrow}>账户登录</Text>
          ...
          >
            ...
            {!challengeId && localE2E ? (
              <View style={styles.previewNote}>
                <Ionicons name="flask-outline" size={17} color={colors.green} />
            ...
            {challengeId ? (
              <>
                <Text style={styles.label}>{localE2E ? '本机验证码' : '短信验证码'}</Text>
                <View style={styles.inputRow}>
                ...
                <View style={styles.sentLine}>
                  ...
                  <Text style={styles.sentText}>{localCodeFilled ? '本机验收码已填入，直接验证即可。' : `验证码已发送；约 ${resendAfter ?? 60} 秒后可重新获取。`}</Text>
            ...
            {mode === 'register' ? (
              <Pressable style={styles.consentRow} onPress={() => setConsented((value) => !value)}>
                ...
                <Pressable onPress={() => openDocument(documents?.terms.url)} hitSlop={6}>
                  <Text style={styles.documentLink}>用户协议</Text>
                </Pressable>
                <Text style={styles.consentText}>和</Text>
                <Pressable onPress={() => openDocument(documents?.privacy.url)} hitSlop={6}>
            ...
            <View style={styles.securityNote}>
              ...
              <Text style={styles.securityText}>验证码不会保存；登录凭证只存放在本机系统安全存储中，并由服务端自动轮换。</Text>
            </View>
          </ScrollView>
          ...
          <View style={styles.actionArea}>
            <Pressable
              ...
              onPress={challengeId ? verify : requestCode}
...
const styles = StyleSheet.create({
  ...
  handle: { width: 42, height: 5, borderRadius: 3, alignSelf: 'center', backgroundColor: '#D4DDD7', marginBottom: 16 },
  ...
  inputRow: { minHeight: 54, flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 15, marginBottom: 14, borderWidth: 1, borderColor: colors.line, borderRadius: 17, backgroundColor: '#FCFEFC' },
  ...
  previewNote: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 11, marginTop: -5, marginBottom: 14, borderRadius: 13, backgroundColor: colors.greenSoft },
  previewNoteText: { flex: 1, color: colors.greenDark, fontSize: 11, lineHeight: 17 },
  ...
  changePhone: { color: colors.green, fontSize: 11, fontWeight: '800' },
  ...
  documentLink: { color: colors.green, fontSize: 12, fontWeight: '800' },
  ...
  primaryButton: { minHeight: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 17, backgroundColor: colors.green },
  primaryButtonDisabled: { opacity: 0.42 },
  primaryText: { color: colors.surface, fontSize: 15, fontWeight: '900' },
  securityNote: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, padding: 13, marginTop: 8, borderRadius: 14, backgroundColor: colors.greenSoft },
  securityText: { flex: 1, color: colors.greenDark, fontSize: 11, lineHeight: 17 },
});
