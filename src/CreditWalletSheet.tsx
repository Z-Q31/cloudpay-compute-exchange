...
function creditForYuan(yuan: number) { return compact((yuan / 1.002).toFixed(6)); }
    ...
    <View style={styles.backdrop}><View style={styles.sheet}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.balanceCard}>
          ...
          <Text style={styles.balanceValue}>{balance ? compact(balance.available) : '—'}</Text>
          <Text style={styles.balanceUnit}>KAI 卡时</Text>
          <View style={styles.balanceDivider} />
          <View style={styles.balanceFacts}><BalanceFact label="订单预留" value={balance ? compact(balance.reserved) : '—'} /><BalanceFact label="待结算" value={balance ? compact(balance.supplierReceivable) : '—'} /></View>
        </View>
        ...
        {distributionPolicy.nativeTopups ? <>
          ...
          {error ? <View style={styles.notice}><Ionicons name="information-circle-outline" size={18} color={colors.green} /><Text style={styles.noticeText}>{error}</Text></View> : null}
          ...
          <View style={styles.historyHeader}><Text style={styles.sectionTitle}>充值记录</Text><Pressable onPress={() => void refresh()}><Ionicons name="refresh" size={19} color={colors.green} /></Pressable></View>
          {loading && topups.length === 0 ? <ActivityIndicator style={styles.loader} color={colors.green} /> : null}
          {topups.length === 0 && !loading ? <View style={styles.empty}><Text style={styles.emptyText}>还没有充值记录</Text></View> : topups.map((topup) => <View key={topup.id} style={styles.record}>
            <View style={styles.recordIcon}><Ionicons name={topup.provider === 'alipay' ? 'logo-alipay' : 'logo-wechat'} size={22} color={colors.green} /></View>
            <View style={styles.recordCopy}><Text style={styles.recordTitle}>¥{topup.amountCny} · {compact(topup.creditAmount)} 卡时</Text><Text style={styles.recordMeta}>{dateTime(topup.createdAt)} · {statusLabel[topup.status]}</Text></View>
          ...
          <View style={styles.managedIcon}><Ionicons name="business-outline" size={23} color={colors.green} /></View>
...
function ProviderChoice({ label, icon, active, enabled, onPress }: Readonly<{ label: string; icon: 'logo-alipay' | 'logo-wechat'; active: boolean; enabled: boolean; onPress: () => void }>) { return <Pressable disabled={!enabled} onPress={onPress} style={[styles.provider, active && styles.providerActive, !enabled && styles.providerDisabled]}><Ionicons name={icon} size={22} color={active ? colors.green : colors.muted} /><Text style={[styles.providerText, active && styles.providerTextActive]}>{label}</Text>{!enabled ? <Text style={styles.unavailable}>未接通</Text> : null}</Pressable>; }

const styles = StyleSheet.create({
  backdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(10,24,17,0.42)' }, sheet: { height: '94%', borderTopLeftRadius: 30, borderTopRightRadius: 30, overflow: 'hidden', backgroundColor: colors.canvas }, handle: { width: 42, height: 5, borderRadius: 3, alignSelf: 'center', marginTop: 9, backgroundColor: '#D2DDD5' },
  header: { minHeight: 76, paddingHorizontal: 18, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, eyebrow: { color: colors.green, fontSize: 9, fontWeight: '900', letterSpacing: 1.4 }, title: { color: colors.ink, fontSize: 25, fontWeight: '900', marginTop: 3 }, close: { width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface },
  content: { padding: 17, paddingBottom: 40 }, balanceCard: { padding: 20, borderRadius: 24, backgroundColor: '#10321F' }, balanceLabel: { color: '#A9D5B4', fontSize: 10 }, balanceValue: { color: colors.surface, fontSize: 39, fontWeight: '900', letterSpacing: -1, marginTop: 5 }, balanceUnit: { color: '#B9F2C8', fontSize: 11, marginTop: 1 }, balanceDivider: { height: 1, backgroundColor: '#315B40', marginVertical: 17 }, balanceFacts: { flexDirection: 'row', gap: 60 }, factLabel: { color: '#A9D5B4', fontSize: 9 }, factValue: { color: colors.surface, fontSize: 17, fontWeight: '900', marginTop: 4 },
  sectionTitle: { color: colors.ink, fontSize: 18, fontWeight: '900', marginTop: 23 }, sectionHelp: { color: colors.muted, fontSize: 10, lineHeight: 16, marginTop: 5 }, amountGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 9, marginTop: 13 }, amount: { width: '48.5%', minHeight: 70, padding: 12, borderWidth: 1, borderColor: colors.line, borderRadius: 16, backgroundColor: colors.surface }, amountActive: { borderColor: colors.green, backgroundColor: colors.greenSoft }, amountText: { color: colors.ink, fontSize: 17, fontWeight: '900' }, amountTextActive: { color: colors.greenDark }, amountCredit: { color: colors.muted, fontSize: 9, marginTop: 6 }, amountCreditActive: { color: colors.green },
  providers: { flexDirection: 'row', gap: 9, marginTop: 13 }, provider: { flex: 1, minHeight: 54, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 8, borderWidth: 1, borderColor: colors.line, borderRadius: 16, backgroundColor: colors.surface }, providerActive: { borderColor: colors.green, backgroundColor: colors.greenSoft }, providerDisabled: { opacity: 0.52 }, providerText: { flex: 1, color: colors.muted, fontSize: 11, fontWeight: '800' }, providerTextActive: { color: colors.greenDark }, unavailable: { color: colors.subtle, fontSize: 8 },
  conversion: { padding: 15, marginTop: 13, borderRadius: 17, backgroundColor: colors.surface }, conversionLabel: { color: colors.muted, fontSize: 9 }, conversionValue: { color: colors.greenDark, fontSize: 18, fontWeight: '900', marginTop: 5 }, conversionRule: { color: colors.subtle, fontSize: 9, marginTop: 5 }, notice: { flexDirection: 'row', gap: 8, padding: 11, marginTop: 12, borderRadius: 14, backgroundColor: colors.greenSoft }, noticeText: { flex: 1, color: colors.greenDark, fontSize: 10, lineHeight: 16 },
  primary: { minHeight: 53, marginTop: 13, borderRadius: 17, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, primaryText: { color: colors.surface, fontSize: 13, fontWeight: '900' }, disabled: { opacity: 0.45 }, safety: { color: colors.muted, fontSize: 9, lineHeight: 15, textAlign: 'center', marginTop: 10 },
  managedAccount: { minHeight: 88, marginTop: 18, padding: 15, borderRadius: 18, flexDirection: 'row', alignItems: 'center', backgroundColor: colors.greenSoft }, managedIcon: { width: 44, height: 44, borderRadius: 15, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface }, managedCopy: { flex: 1, marginLeft: 12 }, managedTitle: { color: colors.greenDark, fontSize: 14, fontWeight: '900' }, managedText: { color: colors.green, fontSize: 10, lineHeight: 16, marginTop: 4 },
  historyHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, loader: { marginTop: 20 }, empty: { padding: 22, marginTop: 12, borderRadius: 17, alignItems: 'center', backgroundColor: colors.surface }, emptyText: { color: colors.muted, fontSize: 11 }, record: { minHeight: 72, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.line }, recordIcon: { width: 42, height: 42, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft }, recordCopy: { flex: 1, marginLeft: 11 }, recordTitle: { color: colors.ink, fontSize: 12, fontWeight: '900' }, recordMeta: { color: colors.muted, fontSize: 9, marginTop: 5 }, resume: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 11, backgroundColor: colors.greenSoft }, resumeText: { color: colors.green, fontSize: 10, fontWeight: '900' },
});
