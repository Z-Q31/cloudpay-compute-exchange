...
import { cnyPrice, creditUnitPrice } from './format';
...
export function ListingPublishSheet({ visible, offerId, onClose, onPublished }: Readonly<{
  ...
  useEffect(() => {
    if (!visible || !offer || !resource || !durationFits(days)) return;
    let active = true;
  ...
  return <Modal visible={visible} animationType="slide" transparent onRequestClose={closeSafely}>
    <KeyboardAvoidingView style={styles.backdrop} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={styles.sheet}>
        ...
        {loading ? <View style={styles.center}><ActivityIndicator color={colors.green} /><Text style={styles.loadingText}>正在读取双审结果…</Text></View> : null}
        {!loading ? <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} showsVerticalScrollIndicator={false}>
          ...
          {offer && resource && approvedPrice ? <>
            <View style={styles.approvedCard}>
              <View style={styles.approvedTop}><View><Text style={styles.approvedLabel}>双审通过</Text><Text style={styles.offerTitle}>{offer.title}</Text></View><Ionicons name="shield-checkmark" size={30} color="#B9F2C8" /></View>
              <Text style={styles.price}>{creditUnitPrice(approvedPrice)} <Text style={styles.priceUnit}>KAI 卡时 / {offer.nativeUnit}</Text></Text>
            ...
            <Text style={styles.fieldHint}>已验真容量 {displayQuantity(scaled(resource.capacityTotal) ?? 0n)} {resource.capacityUnit} · 最低 {displayQuantity(scaled(offer.minimumQuantity) ?? 0n)}</Text>
            ...
            <View style={styles.summary}>
              ...
              <SummaryRow label="单价" value={`${creditUnitPrice(approvedPrice)} KAI 卡时`} />
            </View>
            <Text style={styles.auditLimit}>审核有效至 {offer.auditValidUntil ? dateLabel(new Date(offer.auditValidUntil)) : '—'}</Text>
            <Pressable disabled={submitting || checkingWindow || availability?.status !== 'available'} onPress={() => void publish()} style={[styles.primary, (submitting || checkingWindow || availability?.status !== 'available') && styles.disabled]}>{submitting ? <><ActivityIndicator color={colors.surface} /><Text style={styles.primaryText}>正在确认挂牌…</Text></> : <><Text style={styles.primaryText}>{checkingWindow ? '正在确认时段' : availability?.status === 'window_conflict' ? '请先更换时间' : '确认挂牌'}</Text><Ionicons name="arrow-forward" size={18} color={colors.surface} /></>}</Pressable>
          </> : null}
        </ScrollView> : null}
      </View>
...
function WindowAvailability({ availability, checking, error, days, onRetry, onUseNext }: Readonly<{
  ...
  if (checking) return <View style={styles.windowCard}><ActivityIndicator size="small" color={colors.green} /><Text style={styles.windowText}>正在确认这段时间…</Text></View>;
