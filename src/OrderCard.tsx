...
const statusCopy: Record<CloudPayOrder['status'], { label: string; color: string; background: string }> = {
  reserved: { label: '待接单', color: colors.amber, background: '#FFF4D4' },
  confirmed: { label: '已接单', color: colors.green, background: colors.greenSoft },
  provisioning: { label: '开通中', color: '#2563A7', background: '#EAF3FF' },
  ready: { label: '已就绪', color: '#2563A7', background: '#EAF3FF' },
    ...
    <Pressable onPress={onPress} accessibilityRole="button" accessibilityLabel={`查看订单 ${order.orderNumber}`}>
      <Card style={styles.card}>
        <View style={styles.top}>
          <View style={styles.icon}><Ionicons name="hardware-chip-outline" size={23} color={colors.green} /></View>
        ...
        <View style={styles.amountRow}>
          <View><Text style={styles.amountLabel}>订单卡时</Text><Text style={styles.amount}>{trim(order.totalCredits)}</Text></View>
        ...
        <View style={styles.footer}>
          ...
          <View style={styles.open}><Text style={styles.openText}>{order.actions.length > 0 ? '去处理' : '详情'}</Text><Ionicons name="chevron-forward" size={15} color={colors.green} /></View>
        </View>
      </Card>
    </Pressable>
  );
...
const styles = StyleSheet.create({
  ...
  icon: { width: 46, height: 46, borderRadius: 15, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  ...
  amountLabel: { color: colors.muted, fontSize: 9 }, amount: { color: colors.greenDark, fontSize: 22, fontWeight: '900', marginTop: 2 },
  ...
  openText: { color: colors.green, fontSize: 11, fontWeight: '900' },
});
