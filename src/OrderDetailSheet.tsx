...
import {
  acceptCloudPayDelivery, approveAftercareRefund, approveCloudPayDeliveryRefund,
  cancelCloudPayOrder, confirmCloudPayOrder, contestAftercareRefund, escalateAftercareRefund,
  escalateCloudPayDeliveryDispute, loadAftercareRefund, loadCloudPayDelivery,
  loadCloudPayDeliveryIssue, loadCloudPaySupplierSettlement, reportCloudPayDeliveryIssue, requestAftercareRefund,
  startCloudPayDelivery, startCloudPayDeliveryRework, submitCloudPayDelivery, loadCloudPayOrder,
  type AftercareRefund, type CloudPayOrder, type OrderDelivery, type OrderDeliveryIssue, type SupplierSettlement,
} from './api';
import { ApiError } from './api-client';
import { clearDeliveryDraft, loadDeliveryDraft, saveDeliveryDraft } from './delivery-draft';
...
export function OrderDetailSheet({ order, onClose, onChanged }: Readonly<{
  ...
  const [requestedResolution, setRequestedResolution] = useState<'rework' | 'refund'>('rework');
  ...
  const [draftReadyForOrderId, setDraftReadyForOrderId] = useState<string | null>(null);
  const openedOrderId = useRef<string | null>(null);
  ...
  const loadRelated = useCallback(async (target: CloudPayOrder) => {
    ...
    try {
      if (['acceptance_pending', 'disputed', 'accepted', 'refunded', 'closed'].includes(target.status)) {
        try { setDelivery((await loadCloudPayDelivery(target.id)).delivery); }
      ...
      if (['accepted', 'refunded', 'closed'].includes(target.status)) {
      ...
      if (target.side === 'provider' && target.status === 'closed') {
  ...
  useEffect(() => {
    ...
    else { setDelivery(null); setIssue(null); setAftercare(null); setSettlement(null); }
    return undefined;
  }, [loadRelated, order]);

  useEffect(() => {
    const orderId = order?.id ?? null;
    if (openedOrderId.current === orderId) return undefined;
    openedOrderId.current = orderId;
    setEndpoint(''); setUsername(''); setPassword(''); setInstructions('');
    setIssueDescription(''); setAftercareDescription(''); setAftercareCredits(''); setProviderResponse('');
    setDraftReadyForOrderId(null);
    if (!orderId || order?.side !== 'provider') return undefined;
    let live = true;
    void loadDeliveryDraft(orderId).then((draft) => {
      if (!live) return;
      if (draft) {
        setEndpoint(draft.endpoint); setUsername(draft.username); setInstructions(draft.instructions);
        setNotice('已恢复上次填写的交付内容，临时密码请重新填写。');
      }
      setDraftReadyForOrderId(orderId);
    }).catch(() => {
      if (live) setDraftReadyForOrderId(orderId);
    });
    return () => { live = false; };
  }, [order?.id, order?.side]);

  useEffect(() => {
    if (!currentOrder || currentOrder.side !== 'provider'
      || currentOrder.id !== draftReadyForOrderId) return undefined;
    if (currentOrder.status !== 'provisioning') {
      void clearDeliveryDraft(currentOrder.id).catch(() => undefined);
      return undefined;
    }
    const timer = setTimeout(() => {
      const draft = { endpoint, username, instructions };
      const hasContent = Object.values(draft).some((value) => value.length > 0);
      const operation = hasContent
        ? saveDeliveryDraft(currentOrder.id, draft)
        : clearDeliveryDraft(currentOrder.id);
      void operation.catch(() => undefined);
    }, 450);
    return () => clearTimeout(timer);
  }, [currentOrder, draftReadyForOrderId, endpoint, instructions, username]);
  ...
  const run = async (
    ...
    try {
      ...
      if (result.order.status !== 'provisioning') {
        await clearDeliveryDraft(result.order.id).catch(() => undefined);
        setEndpoint(''); setUsername(''); setPassword(''); setInstructions('');
      }
      await onChanged();
        ...
        try {
          ...
          if (changed) {
            ...
            if (latest.status !== 'provisioning') {
              await clearDeliveryDraft(latest.id).catch(() => undefined);
              setEndpoint(''); setUsername(''); setPassword(''); setInstructions('');
            }
  ...
  const close = () => {
    if (currentOrder?.side === 'provider' && currentOrder.status === 'provisioning') {
      const draft = { endpoint, username, instructions };
      const operation = Object.values(draft).some((value) => value.length > 0)
        ? saveDeliveryDraft(currentOrder.id, draft)
        : clearDeliveryDraft(currentOrder.id);
      void operation.catch(() => undefined);
    }
  ...
  const canRequestAftercare = currentOrder.side === 'buyer' && currentOrder.status === 'accepted' && !aftercare
    && Boolean(currentOrder.settlementAvailableAt && new Date(currentOrder.settlementAvailableAt) > new Date());
  const deliveryReady = endpoint.trim().length >= 3 && instructions.trim().length >= 3;
  ...
  const providerSettlementCredits = aftercare?.status === 'succeeded'
    ? remainingCreditAmount(currentOrder.totalCredits, aftercare.creditAmount) : currentOrder.totalCredits;
        ...
        <View style={styles.sheet}>
          ...
          >
            <View style={styles.summary}>
              ...
              <Text style={styles.credits}>{trim(currentOrder.totalCredits)} <Text style={styles.unit}>KAI 卡时</Text></Text>
            ...
            <View style={styles.timeline}>
              ...
              <Timeline label="提供方接单" value={formatTime(currentOrder.confirmedAt)} done={Boolean(currentOrder.confirmedAt)} />
              <Timeline label="提交交付" value={formatTime(currentOrder.deliveryReadyAt)} done={Boolean(currentOrder.deliveryReadyAt)} />
              <Timeline label="买方验收" value={formatTime(currentOrder.acceptedAt)} done={Boolean(currentOrder.acceptedAt)} last />
            </View>
            ...
            {delivery ? <View style={styles.detailCard}>
              <View style={styles.cardHeading}><Text style={styles.sectionTitle}>交付信息</Text><Text style={styles.attempt}>第 {delivery.attemptNumber} 次交付</Text></View>
              {Object.entries(delivery.details).map(([key, value]) => <View key={key} style={styles.detailRow}><Text style={styles.detailKey}>{deliveryLabel(key)}</Text><Text selectable style={styles.detailValue}>{String(value)}</Text></View>)}
            </View> : null}

            {currentOrder.actions.length > 0 ? <View style={styles.actionCard}>
              <Text style={styles.sectionTitle}>现在要做</Text>
              {currentOrder.actions.includes('confirm_order') ? <PrimaryButton label="确认接单" busy={busy}
                onPress={() => {
              ...
              {currentOrder.actions.includes('start_delivery') ? <PrimaryButton label="开始交付" busy={busy}
                onPress={() => {
                  const signature = `start-delivery:${currentOrder.id}`;
                  void run(() => startCloudPayDelivery(currentOrder.id, actionRequestId(signature)),
                    { signature, action: 'start_delivery', notice: '订单已经进入交付，填写交付内容后即可提交。' });
                }} /> : null}
              {currentOrder.actions.includes('submit_delivery') ? <View>
                <Text style={styles.help}>填写买方实际拿到的连接方式。提交后进入验收。</Text>
                <Field label="访问地址" value={endpoint} onChange={setEndpoint} placeholder="例如 https://console.example.com" />
                <Field label="登录账号（选填）" value={username} onChange={setUsername} placeholder="账号或实例 ID" />
                <Field label="临时密码（选填）" value={password} onChange={setPassword} placeholder="建议买方首次登录后修改" secure />
                <Field label="使用说明" value={instructions} onChange={setInstructions} placeholder="写明登录、调用或验收步骤" multiline />
                <Text style={styles.draftNote}>离开后会保留地址、账号和说明；临时密码不会保存。</Text>
              </View> : null}
              {currentOrder.actions.includes('accept_delivery') ? <PrimaryButton label="确认验收" busy={busy}
                onPress={() => confirm('确认验收', `验收后 ${trim(currentOrder.totalCredits)} KAI 卡时进入提供方待结算账户，并开始 7 天售后期。`, '确认验收', () => acceptCloudPayDelivery(currentOrder.id))} /> : null}
              {currentOrder.actions.includes('report_delivery_issue') ? <View style={styles.issueForm}>
                <Text style={styles.help}>交付不符合约定时，选择希望提供方重新交付，或申请全额退款。</Text>
                <View style={styles.choiceRow}>
                  <Choice active={requestedResolution === 'rework'} label="重新交付" onPress={() => setRequestedResolution('rework')} />
                  <Choice active={requestedResolution === 'refund'} label="全额退款" onPress={() => setRequestedResolution('refund')} />
                </View>
                <Field label="问题说明" value={issueDescription} onChange={setIssueDescription} placeholder="写清不符合约定的地方" multiline />
                <SecondaryButton label="提交交付问题" busy={busy} disabled={issueDescription.trim().length < 5}
                  onPress={() => void run(() => reportCloudPayDeliveryIssue(currentOrder.id, requestedResolution, issueDescription.trim()))} />
              </View> : null}
            </View> : null}

            {currentOrder.side === 'provider' && currentOrder.status === 'accepted' && currentOrder.settlementAvailableAt ? (
              <View style={styles.settlementCard}>
                <View style={styles.settlementHeading}>
                  <View><Text style={styles.settlementEyebrow}>{aftercare && ['pending', 'escalated'].includes(aftercare.status) ? '结算已暂停' : '待结算卡时'}</Text><Text style={styles.settlementAmount}>{trim(providerSettlementCredits)}</Text></View>
                ...
                <Text style={styles.settlementText}>{aftercare && ['pending', 'escalated'].includes(aftercare.status)
                  ...
                  : aftercare?.status === 'succeeded'
                    ? `已补偿买方 ${trim(aftercare.creditAmount)} 卡时，剩余卡时按原日期到账。`
                    : '买方验收后的 7 天为售后期。期间没有补偿或争议，系统会自动到账。'}</Text>
              </View>
            ) : null}

            {currentOrder.side === 'provider' && currentOrder.status === 'closed' && settlement ? (
              <View style={styles.settledCard}>
                <View><Text style={styles.settledEyebrow}>已转入可用卡时</Text><Text style={styles.settledAmount}>{trim(settlement.creditAmount)}</Text></View>
            ...
            {issue ? <View style={styles.detailCard}>
              ...
              {issue.actions.includes('approve_refund') ? <PrimaryButton label="同意全额退款" busy={busy} tone="danger"
                onPress={() => confirm('同意全额退款', `将退回买方 ${trim(currentOrder.totalCredits)} KAI 卡时，并恢复本单预留资源数量。`, '确认退款', () => approveCloudPayDeliveryRefund(currentOrder.id))} /> : null}
            ...
            {canRequestAftercare ? <View style={styles.detailCard}>
              ...
              <Field label="补偿卡时" value={aftercareCredits} onChange={setAftercareCredits} placeholder={`最多 ${trim(currentOrder.totalCredits)}`} keyboardType="decimal-pad" />
              {requestedAftercareMicros !== null && totalOrderMicros !== null && requestedAftercareMicros > totalOrderMicros ? <Text style={styles.fieldError}>不能超过本单 {trim(currentOrder.totalCredits)} 卡时</Text> : null}
              ...
              <PrimaryButton label="提交补偿申请" busy={busy} tone="danger" disabled={!aftercareReady}
                onPress={() => confirm('提交补偿申请', `申请退回 ${trim(aftercareCredits)} KAI 卡时，提供方待结算金额会先暂停。`, '确认提交', () => requestAftercareRefund(currentOrder.id, aftercareDescription.trim(), aftercareCredits.trim()))} />
            </View> : null}
            ...
            {aftercare ? <View style={styles.detailCard}>
              ...
              <View style={styles.refundAmount}><Text style={styles.refundLabel}>申请退回</Text><Text style={styles.refundValue}>{trim(aftercare.creditAmount)} KAI 卡时</Text></View>
              ...
              {aftercare.actions.includes('approve_refund') ? <PrimaryButton label="同意补偿" busy={busy} tone="danger" onPress={() => confirm('同意补偿', `将从待结算账户退回买方 ${trim(aftercare.creditAmount)} KAI 卡时，剩余卡时按原日期结算。`, '确认补偿', () => approveAftercareRefund(currentOrder.id))} /> : null}
          ...
          {currentOrder.actions.includes('submit_delivery') ? <View style={styles.actionFooter}>
            <PrimaryButton label="提交交付" busy={busy} disabled={!deliveryReady} onPress={() => void run(() => submitCloudPayDelivery(currentOrder.id, {
              endpoint: endpoint.trim(), instructions: instructions.trim(),
...
function AftercarePill({ status }: Readonly<{ status: AftercareRefund['status'] }>) { const map = { pending: '待提供方处理', escalated: '平台处理中', succeeded: '已补偿', rejected: '未支持补偿' } as const; return <View style={[styles.pill, status === 'escalated' && styles.pillWarn, status === 'rejected' && styles.pillMuted]}><Text style={styles.pillText}>{map[status]}</Text></View>; }
...
const styles = StyleSheet.create({
  backdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(10,24,17,0.42)' }, sheet: { height: '94%', borderTopLeftRadius: 30, borderTopRightRadius: 30, backgroundColor: colors.canvas, overflow: 'hidden' }, handle: { width: 42, height: 5, borderRadius: 3, alignSelf: 'center', marginTop: 9, backgroundColor: '#D2DDD5' },
  header: { minHeight: 75, paddingHorizontal: 18, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, headerCopy: { flex: 1, marginRight: 12 }, eyebrow: { color: colors.green, fontSize: 9, fontWeight: '900', letterSpacing: 1.2 }, title: { color: colors.ink, fontSize: 21, fontWeight: '900', marginTop: 3 }, close: { width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface },
  scroll: { flex: 1 }, content: { padding: 17, paddingBottom: 24 }, actionFooter: { paddingHorizontal: 17, paddingTop: 2, paddingBottom: 14, borderTopWidth: 1, borderTopColor: colors.line, backgroundColor: colors.surface }, summary: { padding: 18, borderRadius: 23, backgroundColor: '#0B4F2B' }, summaryTop: { flexDirection: 'row', justifyContent: 'space-between' }, status: { color: '#B8F8CB', fontSize: 11, fontWeight: '900' }, number: { color: '#A9CDB5', fontSize: 9 }, credits: { color: colors.surface, fontSize: 31, fontWeight: '900', marginTop: 17 }, unit: { fontSize: 12, color: '#C7EAD2' }, meta: { color: '#C7EAD2', fontSize: 10, marginTop: 6 },
  ...
  loader: { marginVertical: 20 }, notice: { flexDirection: 'row', gap: 8, padding: 12, marginTop: 14, borderRadius: 14, backgroundColor: colors.greenSoft }, noticeText: { flex: 1, color: colors.greenDark, fontSize: 11, lineHeight: 17 }, error: { flexDirection: 'row', gap: 8, padding: 12, marginTop: 14, borderRadius: 14, backgroundColor: '#FDECEC' }, errorText: { flex: 1, color: colors.red, fontSize: 11, lineHeight: 17 }, actionCard: { padding: 16, marginTop: 15, borderRadius: 22, backgroundColor: '#EFF8F1' }, detailCard: { padding: 16, marginTop: 15, borderWidth: 1, borderColor: colors.line, borderRadius: 22, backgroundColor: colors.surface },
  help: { color: colors.muted, fontSize: 11, lineHeight: 18, marginTop: 8 }, field: { marginTop: 13 }, fieldLabel: { color: colors.ink, fontSize: 10, fontWeight: '800', marginBottom: 6 }, fieldError: { color: colors.red, fontSize: 9, marginTop: 6 }, input: { minHeight: 48, paddingHorizontal: 13, borderWidth: 1, borderColor: colors.line, borderRadius: 15, color: colors.ink, backgroundColor: colors.surface }, multiline: { minHeight: 88, paddingTop: 12, textAlignVertical: 'top' }, draftNote: { color: colors.muted, fontSize: 9, lineHeight: 15, marginTop: 10 }, primary: { minHeight: 50, marginTop: 13, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, danger: { backgroundColor: '#B73A35' }, primaryText: { color: colors.surface, fontSize: 13, fontWeight: '900' }, disabled: { opacity: 0.48 }, secondary: { minHeight: 48, marginTop: 11, borderWidth: 1, borderColor: colors.green, borderRadius: 16, alignItems: 'center', justifyContent: 'center' }, secondaryText: { color: colors.green, fontSize: 12, fontWeight: '900' },
  choiceRow: { flexDirection: 'row', gap: 9, marginTop: 12 }, choice: { flex: 1, minHeight: 46, paddingHorizontal: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderWidth: 1, borderColor: colors.line, borderRadius: 14, backgroundColor: colors.surface }, choiceActive: { borderColor: colors.green, backgroundColor: colors.greenSoft }, choiceText: { color: colors.muted, fontSize: 11, fontWeight: '800' }, choiceTextActive: { color: colors.greenDark }, issueForm: { marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#D6E8DA' },
  cardHeading: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }, attempt: { color: colors.green, fontSize: 9, fontWeight: '900', marginTop: 6 }, detailRow: { flexDirection: 'row', gap: 15, marginTop: 13 }, detailKey: { width: 72, color: colors.muted, fontSize: 10 }, detailValue: { flex: 1, color: colors.ink, fontSize: 11, lineHeight: 17, fontWeight: '700', textAlign: 'right' }, reasonLabel: { color: colors.green, fontSize: 9, fontWeight: '900', marginTop: 16 }, reason: { color: colors.ink, fontSize: 12, lineHeight: 19, marginTop: 8 },
  ...
  aftercareMeta: { color: colors.muted, fontSize: 9, marginTop: 4 }, pill: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 999, backgroundColor: colors.greenSoft }, pillWarn: { backgroundColor: '#FFF4D4' }, pillMuted: { backgroundColor: colors.canvas }, pillText: { color: colors.greenDark, fontSize: 9, fontWeight: '900' }, refundAmount: { flexDirection: 'row', justifyContent: 'space-between', padding: 12, marginTop: 14, borderRadius: 14, backgroundColor: colors.greenSoft }, refundLabel: { color: colors.muted, fontSize: 10 }, refundValue: { color: colors.greenDark, fontSize: 12, fontWeight: '900' }, waitText: { color: colors.amber, fontSize: 10, lineHeight: 16, marginTop: 14 },
});
