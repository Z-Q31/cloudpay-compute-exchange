...
export function ResourceEvidenceSheet({ resource, canManage, onClose, onChanged }: Readonly<{
  ...
  const closeSafely = () => {
    if (uploadInFlightRef.current || submitInFlightRef.current) return;
    onClose();
  };
        ...
        <View style={styles.sheet}>
          ...
          <View style={styles.header}>
            <View style={styles.headerCopy}>
              <Text style={styles.eyebrow}>资源验真</Text>
              <Text style={styles.title}>{copy.headerTitle}</Text>
              <Text style={styles.resourceName}>{resource.productCode} · {resource.region}</Text>
            </View>
            <Pressable disabled={Boolean(busyCategory) || submitting} onPress={closeSafely} style={styles.closeButton}><Ionicons name="close" size={23} color={colors.ink} /></Pressable>
          </View>
          <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            {loading && !checklist ? <ActivityIndicator color={colors.green} style={styles.loader} /> : null}
            {!loading && !checklist && error ? (
              <View style={styles.loadError}>
                <Ionicons name="cloud-offline-outline" size={30} color={colors.green} />
            ...
            {checklist ? (
              <>
                <View style={[styles.reviewBox, correctionNeedsAction && styles.reviewFailed]}>
                  <Ionicons name={review === 'passed' ? 'checkmark-circle' : review === 'under_review' ? 'time' : correctionNeedsAction ? 'alert-circle' : 'documents'} size={24} color={correctionNeedsAction ? colors.red : colors.green} />
                  ...
                  {categories.map((category) => {
                    ...
                    return (
                      <View key={category.key} style={styles.categoryCard}>
                        <View style={styles.categoryIcon}><Ionicons name={category.icon} size={22} color={colors.green} /></View>
                ...
                {review === 'under_review' || review === 'passed' ? <Pressable onPress={onClose} style={styles.doneButton}><Text style={styles.doneText}>完成</Text></Pressable> : null}
              </>
            ) : null}
          </ScrollView>
        </View>
...
const styles = StyleSheet.create({
  ...
  handle: { width: 42, height: 5, alignSelf: 'center', marginBottom: 16, borderRadius: 3, backgroundColor: '#D4DDD7' },
  header: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 14 }, headerCopy: { flex: 1 },
  eyebrow: { color: colors.green, fontSize: 10, fontWeight: '900', letterSpacing: 1.4 }, title: { color: colors.ink, fontSize: 25, fontWeight: '900', marginTop: 4 }, resourceName: { color: colors.muted, fontSize: 11, marginTop: 5 },
  ...
  reviewBox: { padding: 15, borderRadius: 18, flexDirection: 'row', alignItems: 'flex-start', gap: 11, backgroundColor: colors.greenSoft }, reviewFailed: { backgroundColor: '#FFF1F1' }, reviewCopy: { flex: 1 }, reviewTitle: { color: colors.ink, fontSize: 15, fontWeight: '900' }, reviewText: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 4 },
  ...
  categoryIcon: { width: 42, height: 42, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft }, categoryCopy: { flex: 1, marginLeft: 10, marginRight: 8 }, categoryTitleRow: { flexDirection: 'row', justifyContent: 'space-between', gap: 8 }, categoryTitle: { color: colors.ink, fontSize: 13, fontWeight: '900' }, stateText: { fontSize: 9, fontWeight: '900' }, categoryHint: { color: colors.muted, fontSize: 9, lineHeight: 14, marginTop: 3 }, fileName: { color: colors.ink, fontSize: 9, marginTop: 5 },
  fileButton: { minWidth: 64, minHeight: 36, paddingHorizontal: 10, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft }, fileButtonText: { color: colors.greenDark, fontSize: 10, fontWeight: '900' }, progressTrack: { height: 4, marginTop: 7, borderRadius: 2, overflow: 'hidden', backgroundColor: colors.line }, progressFill: { height: 4, borderRadius: 2, backgroundColor: colors.green },
  ...
  submitButton: { minHeight: 52, marginTop: 15, borderRadius: 17, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: colors.green }, submitText: { color: colors.surface, fontSize: 14, fontWeight: '900' },
  ...
  retryButton: { minHeight: 46, marginTop: 15, paddingHorizontal: 28, borderRadius: 15, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green },
