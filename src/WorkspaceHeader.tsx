      ...
      <View style={styles.shell}>
        ...
        <View style={styles.segment}>
          <Pressable onPress={() => switchMode('consumer')} style={[styles.segmentItem, mode === 'consumer' && styles.segmentItemActive]}>
            <Ionicons name="flash-outline" size={16} color={mode === 'consumer' ? colors.surface : colors.muted} />
          ...
          <Pressable onPress={() => switchMode('provider')} style={[styles.segmentItem, mode === 'provider' && styles.segmentItemActive]}>
            <Ionicons name="server-outline" size={16} color={mode === 'provider' ? colors.surface : colors.muted} />
              ...
              {subjects.map((item) => (
                <Pressable key={item.id} onPress={() => onSelectSubject(item.id)} style={[styles.subjectRow, item.selected && styles.subjectRowSelected]}>
                  <View style={[styles.subjectIcon, item.selected && styles.subjectIconSelected]}>
                    <Ionicons name={item.kind === 'organization' ? 'business-outline' : 'person-outline'} size={20} color={item.selected ? colors.surface : colors.green} />
                  </View>
                  ...
                  {item.selected ? <Ionicons name="checkmark-circle" size={22} color={colors.green} /> : null}
                </Pressable>
              ))}
            </View>
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  logo: { width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green },
  ...
  avatar: { width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  avatarText: { color: colors.greenDark, fontSize: 15, fontWeight: '900' },
  segment: { flexDirection: 'row', padding: 4, marginTop: 10, borderRadius: 16, backgroundColor: '#E6ECE8' },
  segmentItem: { flex: 1, minHeight: 38, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, borderRadius: 12 },
  segmentItemActive: { backgroundColor: colors.green, ...shadows.card },
  ...
  pickerEyebrow: { color: colors.green, fontSize: 10, fontWeight: '900', letterSpacing: 1.5 },
  ...
  subjectRowSelected: { borderColor: colors.green, backgroundColor: '#F7FCF8' },
  subjectIcon: { width: 44, height: 44, borderRadius: 15, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  subjectIconSelected: { backgroundColor: colors.green },
