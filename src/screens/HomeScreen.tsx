...
export function HomeScreen({ snapshot, refreshing, onRefresh, onNavigate, onOpenDemand }: Props) {
  ...
  return (
    <View style={styles.root}>
      ...
      >
        <LinearGradient colors={['#0A4D2D', '#137343', '#1E8B54']} style={styles.radarCard}>
          ...
          <View style={styles.radarCopy}>
            <Text style={styles.radarEyebrow}>算力雷达</Text>
        ...
        <Card style={styles.pulseCard}>
          <View style={styles.pulseHeader}>
            <View>
              <Text style={styles.pulseTitle}>市场脉搏</Text>
              <Text style={styles.pulseCaption}>直接来自 CloudPay 服务端</Text>
            </View>
            <Ionicons name="radio-outline" size={24} color={colors.green} />
          </View>
        ...
        <SectionTitle title="用一个目的开始" caption="不需要先理解产品名，选择你现在要完成的事。" />
          ...
          {missions.map((mission, index) => (
            ...
            >
              <View style={[styles.missionIcon, { backgroundColor: mission.tone }]}>
                <Ionicons name={mission.icon} size={24} color={colors.greenDark} />
              </View>
        ...
        <SectionTitle title="交付时间线" caption="从意图到验收，每一步都留在同一条线上。" action="查看消息" onAction={() => onNavigate('messages')} />
...
const styles = StyleSheet.create({
  ...
  radarCard: { minHeight: 330, borderRadius: 30, padding: 22, overflow: 'hidden', justifyContent: 'space-between' },
  radarGlow: { position: 'absolute', right: -90, top: -70, width: 280, height: 280, borderRadius: 140, backgroundColor: 'rgba(255,255,255,0.07)' },
  radarRingOuter: { position: 'absolute', right: -32, top: 34, width: 230, height: 230, borderRadius: 115, borderWidth: 1, borderColor: 'rgba(255,255,255,0.18)' },
  radarRingInner: { position: 'absolute', right: 22, top: 88, width: 122, height: 122, borderRadius: 61, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  radarSweep: { position: 'absolute', right: 74, top: 86, width: 2, height: 96, backgroundColor: 'rgba(185,255,210,0.62)', transform: [{ rotate: '36deg' }] },
  radarPointOne: { position: 'absolute', right: 86, top: 115, width: 10, height: 10, borderRadius: 5, backgroundColor: '#B8F8CB' },
  radarPointTwo: { position: 'absolute', right: 34, top: 204, width: 7, height: 7, borderRadius: 4, backgroundColor: '#FFF1A8' },
  radarTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', zIndex: 2 },
  radarTime: { color: 'rgba(255,255,255,0.72)', fontSize: 12, fontWeight: '600' },
  radarCopy: { maxWidth: '88%', zIndex: 2, marginTop: 32 },
  radarEyebrow: { color: '#B8F8CB', fontSize: 13, fontWeight: '800', letterSpacing: 1.5, marginBottom: 9 },
  radarTitle: { color: colors.surface, fontSize: 28, lineHeight: 37, fontWeight: '900', letterSpacing: -0.7 },
  radarCaption: { color: 'rgba(255,255,255,0.72)', fontSize: 14, lineHeight: 21, marginTop: 10 },
  radarAction: { zIndex: 2, alignSelf: 'flex-start', minHeight: 46, borderRadius: 16, paddingHorizontal: 18, flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#F4FFF6' },
  radarActionText: { color: colors.greenDark, fontSize: 15, fontWeight: '800' },
  ...
  noticeLine: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, padding: 12, borderRadius: 14, backgroundColor: colors.greenSoft },
  noticeText: { flex: 1, color: colors.greenDark, fontSize: 12, lineHeight: 18 },
  ...
  timelineDotActive: { width: 13, height: 13, borderRadius: 7, backgroundColor: colors.green },
  timelineLine: { position: 'absolute', top: 34, bottom: -28, width: 1, backgroundColor: colors.line },
  timelineCopy: { flex: 1, paddingLeft: 10 },
  timelineIndex: { color: colors.green, fontSize: 10, fontWeight: '900', letterSpacing: 1 },
