  ...
  return (
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}
        ...
        <Text style={styles.caption}>{side === 'provider' ? '接单、交付、验收和结算都在这里处理。' : '购买、使用、验收和售后都在这里查看。'}</Text>
        {snapshot.authenticated && snapshot.orderErrors[side] && orders.length === 0 ? (
          <Card style={styles.emptyCard}>
            <View style={styles.icon}><Ionicons name="cloud-offline-outline" size={35} color={colors.green} /></View>
          ...
          <View>
            {side === 'buyer' && snapshot.aftercareReviews.length > 0 ? <><Text style={styles.sectionTitle}>待处理售后</Text>{snapshot.aftercareReviews.map((review) => (
              <Pressable key={review.refundId} onPress={() => onOpenReview(review)} style={styles.reviewCard}>
                <View style={styles.reviewIcon}><Ionicons name="shield-checkmark-outline" size={22} color={colors.green} /></View>
            ...
            {cursor ? <Pressable disabled={loadingMore} onPress={() => void loadMore()} style={styles.moreButton}>{loadingMore ? <ActivityIndicator color={colors.green} /> : <Text style={styles.moreText}>加载更多订单</Text>}</Pressable> : orders.length > 0 ? <Text style={styles.endText}>已显示全部订单</Text> : null}
          </View>
        ) : <Card style={styles.emptyCard}>
          <View style={styles.icon}><Ionicons name={snapshot.authenticated ? 'receipt-outline' : 'lock-closed-outline'} size={35} color={colors.green} /></View>
...
const styles = StyleSheet.create({
  ...
  eyebrow: { color: colors.green, fontSize: 10, fontWeight: '900', letterSpacing: 1.7, marginTop: 10 },
  ...
  icon: { width: 72, height: 72, borderRadius: 24, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  ...
  action: { minHeight: 48, marginTop: 18, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 16, backgroundColor: colors.green },
  ...
  reviewIcon: { width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  ...
  moreButton: { minHeight: 50, marginTop: 5, borderWidth: 1, borderColor: colors.green, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  moreText: { color: colors.green, fontSize: 13, fontWeight: '900' }, moreError: { color: colors.red, fontSize: 11, lineHeight: 18, textAlign: 'center', marginVertical: 10 },
