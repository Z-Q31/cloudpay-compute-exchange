    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={styles.modalBackdrop}>
        <View style={styles.modalSheet}>
          ...
          {[
            ['公开市场数据', 'App 读取 CloudPay 的公开服务状态与已验真资源档案，不添加演示容量或价格。'],
...
export function ProfileScreen({ snapshot, onSessionChanged, onLogin, onOpenPublish, onOpenCredits }: {
  ...
  const identityCaption = snapshot.authenticated
    ? `${snapshot.user?.phone ?? ''} · ${sessionConfirmed ? '登录已确认' : '本机已保存登录，正在重新连接'}`
    : '登录后查看卡时账户与供应方身份';
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Card style={styles.identityCard}>
          ...
          <Pressable style={styles.loginButton} onPress={accountAction}>
            <Text style={styles.loginButtonText}>{snapshot.authenticated ? '安全退出当前设备' : '在 App 内登录 CloudPay'}</Text>
        ...
        {snapshot.authenticated && currentSubject ? (
          <Card style={styles.subjectCard}>
            <View style={styles.subjectIcon}><Ionicons name={currentSubject.kind === 'organization' ? 'business-outline' : 'person-outline'} size={22} color={colors.green} /></View>
        ...
        {snapshot.authenticated && snapshot.creditBalance ? (
          <Pressable style={styles.creditCard} onPress={onOpenCredits} accessibilityRole="button" accessibilityLabel="打开 KAI 卡时账户">
            <View style={styles.creditTop}><View><Text style={styles.creditLabel}>KAI 卡时</Text><Text style={styles.creditValue}>{snapshot.creditBalance.available.replace(/\.0+$/u, '').replace(/(\.\d*?)0+$/u, '$1')}</Text></View><View style={styles.creditArrow}><Ionicons name="arrow-forward" size={18} color={colors.greenDark} /></View></View>
            <Text style={styles.creditCaption}>可用卡时 · 订单预留 {snapshot.creditBalance.reserved.replace(/\.0+$/u, '').replace(/(\.\d*?)0+$/u, '$1')}</Text>
          </Pressable>
        ) : null}
        ...
        <View style={styles.statusGrid}>
          ...
          <Card style={styles.statusCard}>
            ...
            <Text style={styles.statusValue}>{snapshot.creditBalance ? snapshot.creditBalance.reserved.replace(/\.0+$/u, '').replace(/(\.\d*?)0+$/u, '$1') : '—'}</Text>
            <Text style={styles.statusLabel}>订单预留</Text>
          </Card>
          <Card style={styles.statusCard}>
            <Ionicons name="storefront-outline" size={22} color={colors.amber} />
            <Text style={styles.statusValue}>{snapshot.creditBalance ? snapshot.creditBalance.supplierReceivable.replace(/\.0+$/u, '').replace(/(\.\d*?)0+$/u, '$1') : '—'}</Text>
          ...
          {[
            ...
            <Pressable key={String(label)} style={[styles.menuRow, index === 2 && styles.menuRowLast]} onPress={action as () => void}>
              <View style={styles.menuIcon}>
                <Ionicons name={icon as 'cube-outline'} size={20} color={colors.greenDark} />
              </View>
...
const styles = StyleSheet.create({
  ...
  identityCard: { padding: 18, marginTop: 6, backgroundColor: '#FCFFFC' },
  identityTop: { flexDirection: 'row', alignItems: 'center' },
  avatar: { width: 64, height: 64, borderRadius: 21, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green },
  ...
  identityLabel: { color: colors.green, fontSize: 10, fontWeight: '900', letterSpacing: 1.2 },
  ...
  loginButton: { minHeight: 46, marginTop: 18, borderRadius: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: colors.green },
  ...
  subjectIcon: { width: 50, height: 50, borderRadius: 17, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  subjectCopy: { flex: 1, marginLeft: 12 }, subjectLabel: { color: colors.green, fontSize: 9, fontWeight: '900', letterSpacing: 1.1 }, subjectTitle: { color: colors.ink, fontSize: 16, fontWeight: '900', marginTop: 3 }, subjectMeta: { color: colors.muted, fontSize: 10, marginTop: 4 },
  creditCard: { minHeight: 128, marginTop: 12, padding: 18, borderRadius: 24, backgroundColor: '#10321F' },
  creditTop: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }, creditLabel: { color: '#B9F2C8', fontSize: 10, fontWeight: '900' }, creditValue: { color: colors.surface, fontSize: 34, fontWeight: '900', marginTop: 5 }, creditArrow: { width: 42, height: 42, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: '#B9F2C8' }, creditCaption: { color: '#A9D5B4', fontSize: 10, marginTop: 7 },
  ...
  menuIcon: { width: 38, height: 38, borderRadius: 13, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  ...
  modalHandle: { width: 42, height: 5, borderRadius: 3, alignSelf: 'center', backgroundColor: '#D4DDD7', marginBottom: 16 },
  modalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 },
  modalEyebrow: { color: colors.green, fontSize: 10, fontWeight: '900', letterSpacing: 1.5 },
  ...
  privacyDot: { width: 9, height: 9, borderRadius: 5, marginTop: 5, backgroundColor: colors.green },
  ...
  modalDone: { minHeight: 50, marginTop: 13, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green },
