...
import { providerOrderNeedsAttention, providerOrderSection, providerWorkspaceMetrics } from '../provider-workspace-metrics';
      ...
      >
        {!snapshot.authenticated ? (
          <Card style={styles.loginCard}>
            <View style={styles.loginIcon}><Ionicons name="server-outline" size={35} color={colors.green} /></View>
          ...
          <>
            ...
            <LinearGradient colors={['#0B4F2B', '#187441']} style={styles.hero}>
              ...
              <Pressable onPress={() => onNext(workspace.nextAction.route, workspace.nextAction.entityId)} style={styles.heroAction}>
                <Text style={styles.heroActionText}>{workspace.nextAction.label}</Text>
                <Ionicons name="arrow-forward" size={18} color={colors.greenDark} />
              </Pressable>
            </LinearGradient>

            <View style={styles.metrics}>
              <Card style={styles.metric}><Text style={styles.metricValue}>{metrics.resourceTotal}</Text><Text style={styles.metricLabel}>资源资产</Text></Card>
            ...
            {providerOrders.length > 0 ? (
              <>
                ...
                <Pressable onPress={onAllOrders} style={styles.allOrders}><Text style={styles.allOrdersText}>查看全部提供订单</Text><Ionicons name="arrow-forward" size={16} color={colors.green} /></Pressable>
              </>
            ) : null}
            ...
            <Text style={styles.sectionTitle}>上架路线</Text>
            <Card style={styles.roadmap}>
              {providerSteps(workspace).map((step, index) => (
                <View key={step.label} style={styles.stepRow}>
                  <View style={styles.rail}>
                    ...
                    {index < 3 ? <View style={[styles.stepLine, step.done && styles.stepLineDone]} /> : null}
                  </View>
...
function nextActionCaption(key: string) {
  ...
  if (key === 'create_offer') return '为已验真资源填写服务边界和人民币核价依据。';
...
function providerSteps(workspace: NonNullable<CloudPaySnapshot['providerWorkspace']>) {
  ...
  const firstIncomplete = [supplierDone, resourceDone, offerDone, listingDone].findIndex((value) => !value);
  return [
    { label: '主体开通', meta: supplierDone ? '已通过主体审核' : '确认身份与授权关系', done: supplierDone, current: firstIncomplete === 0 },
    { label: '资源验真', meta: resourceDone ? `${workspace.resources.verified} 份资源已验真` : '自动核验配置与控制权', done: resourceDone, current: firstIncomplete === 1 },
    { label: '方案与双审', meta: offerDone ? '已有获准商品方案' : workspace.offers.expired > 0 ? '审核已到期，需重新提交' : '提交服务边界与价格依据', done: offerDone, current: firstIncomplete === 2 },
    { label: '选择时段并发布', meta: listingDone ? listingProgressLabel(workspace.listings) : '复用审核结果，快速上架', done: listingDone, current: firstIncomplete === 3 },
  ];
}
...
const styles = StyleSheet.create({
  ...
  hero: { minHeight: 280, padding: 21, borderRadius: 28, overflow: 'hidden' },
  heroTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  modePill: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.13)' },
  modeDot: { width: 7, height: 7, borderRadius: 4, marginRight: 6, backgroundColor: '#B8F8CB' },
  modeText: { color: colors.surface, fontSize: 11, fontWeight: '800' }, subjectRole: { color: '#C8EDD4', fontSize: 11, fontWeight: '800' },
  heroEyebrow: { color: '#B8F8CB', fontSize: 11, fontWeight: '900', letterSpacing: 1.5, marginTop: 42 },
  heroTitle: { color: colors.surface, fontSize: 29, lineHeight: 37, fontWeight: '900', letterSpacing: -0.7, marginTop: 8 },
  heroCaption: { color: 'rgba(255,255,255,0.72)', fontSize: 12, lineHeight: 19, marginTop: 8, maxWidth: '90%' },
  heroAction: { alignSelf: 'flex-start', minHeight: 46, marginTop: 18, paddingHorizontal: 17, flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 15, backgroundColor: '#F2FFF5' },
  heroActionText: { color: colors.greenDark, fontSize: 14, fontWeight: '900' },
  ...
  allOrders: { minHeight: 46, marginBottom: 18, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, borderWidth: 1, borderColor: colors.green, borderRadius: 15 },
  allOrdersText: { color: colors.green, fontSize: 12, fontWeight: '900' },
  ...
  stepCurrent: { width: 18, height: 18, borderRadius: 9, marginTop: 27, borderWidth: 5, borderColor: '#B9E4C5', backgroundColor: colors.green },
  stepLine: { position: 'absolute', top: 39, bottom: -37, width: 2, backgroundColor: colors.line }, stepLineDone: { backgroundColor: '#9ED0AC' },
  stepCopy: { flex: 1, marginLeft: 10 }, stepTitle: { color: colors.ink, fontSize: 15, fontWeight: '800' }, stepTitleCurrent: { color: colors.green }, stepMeta: { color: colors.muted, fontSize: 11, marginTop: 4 },
  currentPill: { paddingHorizontal: 8, paddingVertical: 5, borderRadius: 999, backgroundColor: colors.greenSoft }, currentText: { color: colors.green, fontSize: 10, fontWeight: '900' },
  loginCard: { marginTop: 30, padding: 26, alignItems: 'center' }, loginIcon: { width: 72, height: 72, borderRadius: 24, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft },
  ...
  primary: { minHeight: 48, marginTop: 18, paddingHorizontal: 22, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, primaryText: { color: colors.surface, fontSize: 14, fontWeight: '900' },
});
