...
type Mission = Readonly<{
  ...
  title: string;
  caption: string;
  icon: 'flash-outline' | 'leaf-outline' | 'business-outline';
  colors: readonly [string, string];
}>;

const resourceMission: Mission = {
  mode: 'sell', number: '01', tag: '算力资源', title: '添加算力资源',
  caption: '填写资源型号、容量和可用时间。', icon: 'leaf-outline', colors: ['#E7F8EC', '#F3FCF6'],
};

function onboardingMission(status: SupplierStatus): Mission {
  if (status === 'submitted') return {
    mode: 'supplier', number: '01', tag: '入驻审核', title: '查看审核进度',
    caption: '审核通过后即可添加资源。', icon: 'business-outline', colors: ['#F1ECFF', '#F8F5FF'],
  };
  if (status === 'rejected') return {
    mode: 'supplier', number: '01', tag: '补充资料', title: '继续完成入驻',
    caption: '按审核意见修改后重新提交。', icon: 'business-outline', colors: ['#F1ECFF', '#F8F5FF'],
  };
  if (status === 'suspended') return {
    mode: 'supplier', number: '01', tag: '资格状态', title: '查看暂停原因',
    caption: '当前不能新增资源或挂牌。', icon: 'business-outline', colors: ['#F1ECFF', '#F8F5FF'],
  };
  return {
    mode: 'supplier', number: '01', tag: '资源方入驻', title: '成为资源伙伴',
    caption: '提交主体资料，通过后即可添加资源。', icon: 'business-outline', colors: ['#F1ECFF', '#F8F5FF'],
  };
}
...
export function PublishScreen({ snapshot, onLogin, onWorkspaceChanged, onOpenOfferWizard, onOpenListing, onOpenResourceEvidence, openIntent, onIntentOpened, revealOfferId, onOfferRevealed, revealListingId, onListingRevealed }: Readonly<{
  ...
  const progressWritable = snapshot.online && snapshot.sessionState === 'authenticated' && !snapshot.providerWorkspaceError
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setManagedListingId(revealListingId);
      onListingRevealed();
    }, 250);
    return () => clearTimeout(timer);
  }, [listings, onListingRevealed, revealListingId]);

  useEffect(() => {
    if (!snapshot.authenticated || !resumeAfterLogin) return;
    setMode(resumeAfterLogin);
    setResumeAfterLogin(null);
  }, [resumeAfterLogin, snapshot.authenticated]);

  useEffect(() => {
    if (!openIntent || !snapshot.authenticated) return;
    setMode(openIntent);
    onIntentOpened();
  }, [onIntentOpened, openIntent, snapshot.authenticated]);
  ...
  const confirmAbandonDraft = (draft: OfferWizardDraft) => {
    Alert.alert(
      '放弃这份上架草稿？',
      '草稿会从工作台移除，已验真的资源不会删除；以后仍可为这项资源重新创建上架方案。',
      [
          ...
          <View style={styles.listingSection}>
            ...
            {visibleOffers.map((offer) => (
              ...
              <Card style={styles.progressCard}>
                <View style={styles.progressTop}><View style={styles.progressIcon}><Ionicons name="shield-checkmark-outline" size={20} color={colors.green} /></View><View style={styles.progressCopy}><Text style={styles.progressTitle}>{offer.title}</Text><Text style={styles.progressMeta}>{offerStatusLabel[offer.status]}</Text></View><View style={[styles.offerPill, offer.status === 'changes_requested' && styles.attentionPill]}><Text style={[styles.offerPillText, offer.status === 'changes_requested' && styles.attentionText]}>{offerStatusLabel[offer.status]}</Text></View></View>
                ...
                {['changes_requested', 'rejected'].includes(offer.status) ? (
                  <>
                    ...
                    <Pressable onPress={() => withFreshProgress(() => onOpenOfferWizard({ revisionOfferId: offer.id }))} style={[styles.revisionButton, !progressWritable && styles.staleAction]}>
                      <Text style={styles.revisionText}>按意见修改</Text><Ionicons name="arrow-forward" size={15} color={colors.surface} />
                    </Pressable>
                  </>
                ) : null}
                {offer.status === 'approved' && !listings.some((listing) => listing.offerId === offer.id && ['active', 'paused', 'sold_out'].includes(listing.status)) ? <Pressable onPress={() => withFreshProgress(() => onOpenListing(offer.id))} style={[styles.publishListingButton, !progressWritable && styles.staleAction]}><Text style={styles.publishListingText}>发布可售容量</Text><Ionicons name="arrow-forward" size={15} color={colors.surface} /></Pressable> : null}
            ...
            {listings.map((listing) => {
              ...
              return <View key={listing.id} onLayout={(event) => {
                ...
                <View style={styles.stockSummary}>
                  ...
                  <View><Text style={styles.stockLabel}>已预留</Text><Text style={styles.stockValue}>{compact(listing.capacityReserved)}</Text></View>
                  <View><Text style={styles.stockLabel}>已售</Text><Text style={styles.stockValue}>{compact(listing.capacitySold)}</Text></View>
                </View>
                ...
                <Pressable onPress={() => withFreshProgress(() => setManagedListingId(listing.id))} style={[styles.manageButton, !progressWritable && styles.staleAction]}><Text style={styles.manageText}>{progressWritable ? '管理挂牌' : '同步后管理'}</Text><Ionicons name={progressWritable ? 'settings-outline' : 'cloud-offline-outline'} size={15} color={colors.green} /></Pressable>
              </Card></View>;
            })}
            {progressCount === 0 ? <Text style={styles.emptyProgress}>还没有上架方案。</Text> : null}
            <Pressable onPress={() => withFreshProgress(() => onOpenOfferWizard())} style={[styles.newOfferButton, !progressWritable && styles.staleAction]}><Ionicons name="add" size={19} color={colors.surface} /><Text style={styles.newOfferText}>新建上架方案</Text></Pressable>
            </> : null}
          </View>
        ) : null}

        <Text style={styles.eyebrow}>{providerApproved ? '添加资源' : '资源方入驻'}</Text>
        <Text style={styles.hero}>{providerApproved ? '发布新的算力' : supplierStatus === 'submitted' ? '等待入驻审核' : '先完成资源方入驻'}</Text>
        <Text style={styles.lead}>{providerApproved ? '添加新资源，验真后即可提交上架方案。' : '一个账号即可使用算力和提供算力。'}</Text>
          ...
          {missions.map((mission) => (
            <Pressable key={mission.mode} onPress={() => openMission(mission.mode)}>
              <LinearGradient colors={mission.colors} style={styles.card}>
                ...
                <View style={styles.iconBox}><Ionicons name={mission.icon} size={34} color={colors.greenDark} /></View>
...
const styles = StyleSheet.create({
  ...
  cards: { gap: 12 },
  card: { minHeight: 172, paddingHorizontal: 18, paddingVertical: 19, borderRadius: 26, borderWidth: 1, borderColor: '#DCE6DF', flexDirection: 'row', alignItems: 'center', overflow: 'hidden' },
  ...
  tagText: { color: colors.greenDark, fontSize: 10, fontWeight: '900' },
  title: { color: colors.ink, fontSize: 21, lineHeight: 28, fontWeight: '900', marginTop: 9 },
  caption: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 5 },
  arrow: { width: 48, height: 48, borderRadius: 24, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,0.82)' },
  listingSection: { marginTop: 29 }, sectionHeading: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }, sectionTitle: { color: colors.ink, fontSize: 21, fontWeight: '900' }, sectionCaption: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 5, maxWidth: 280 }, progressRefresh: { minWidth: 64, minHeight: 38, paddingHorizontal: 10, borderRadius: 13, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, backgroundColor: colors.greenSoft }, sectionCount: { color: colors.green, fontSize: 10, fontWeight: '900' },
  progressLoadCard: { minHeight: 150, padding: 22, alignItems: 'center', justifyContent: 'center' }, progressLoadTitle: { color: colors.ink, fontSize: 16, fontWeight: '900', marginTop: 10 }, progressLoadText: { color: colors.muted, fontSize: 11, lineHeight: 17, textAlign: 'center', marginTop: 8 }, retryButton: { minHeight: 42, marginTop: 14, paddingHorizontal: 20, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, retryText: { color: colors.surface, fontSize: 12, fontWeight: '900' },
  syncNotice: { minHeight: 48, marginBottom: 10, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 14, flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#FFF8E7' }, syncNoticeText: { flex: 1, color: colors.muted, fontSize: 10, lineHeight: 15 }, syncRetry: { color: colors.green, fontSize: 10, fontWeight: '900' },
  progressCard: { padding: 14, marginBottom: 10 }, progressTop: { flexDirection: 'row', alignItems: 'center' }, progressIcon: { width: 42, height: 42, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenSoft }, progressCopy: { flex: 1, marginLeft: 10 }, progressTitle: { color: colors.ink, fontSize: 13, fontWeight: '900' }, progressMeta: { color: colors.muted, fontSize: 10, marginTop: 4 }, draftPill: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 999, backgroundColor: colors.greenSoft }, draftPillText: { color: colors.green, fontSize: 9, fontWeight: '900' }, offerPill: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 999, backgroundColor: '#FFF4D4' }, offerPillText: { color: colors.amber, fontSize: 9, fontWeight: '900' }, attentionPill: { backgroundColor: '#FDECEC' }, attentionText: { color: colors.red }, draftActions: { marginTop: 12, flexDirection: 'row', gap: 8 }, abandonDraftButton: { minHeight: 41, paddingHorizontal: 16, borderRadius: 13, borderWidth: 1, borderColor: '#F3CACA', flexDirection: 'row', gap: 5, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFF7F7' }, abandonDraftText: { color: colors.red, fontSize: 11, fontWeight: '900' }, continueButton: { minHeight: 41, flex: 1, borderRadius: 13, flexDirection: 'row', gap: 6, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, continueText: { color: colors.surface, fontSize: 11, fontWeight: '900' },
  auditRows: { marginTop: 12, paddingTop: 11, borderTopWidth: 1, borderTopColor: colors.line, flexDirection: 'row', gap: 18 }, auditRow: { flexDirection: 'row', alignItems: 'center' }, auditDot: { width: 7, height: 7, borderRadius: 4, marginRight: 6 }, auditLabel: { color: colors.ink, fontSize: 10, fontWeight: '800' }, auditStatus: { fontSize: 10, fontWeight: '900', marginLeft: 6 }, emptyProgress: { color: colors.muted, fontSize: 11, textAlign: 'center', paddingVertical: 18 }, newOfferButton: { minHeight: 52, marginTop: 3, borderRadius: 17, flexDirection: 'row', gap: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, newOfferText: { color: colors.surface, fontSize: 13, fontWeight: '900' },
  publishListingButton: { minHeight: 43, marginTop: 12, borderRadius: 13, flexDirection: 'row', gap: 6, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, publishListingText: { color: colors.surface, fontSize: 11, fontWeight: '900' }, livePill: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 999, backgroundColor: colors.greenSoft }, livePillText: { color: colors.green, fontSize: 9, fontWeight: '900' }, inactivePill: { backgroundColor: '#FFF4D4' }, inactivePillText: { color: colors.amber }, scheduledPill: { backgroundColor: '#E7F1FF' }, scheduledPillText: { color: colors.blue }, listingWindow: { marginTop: 12, paddingTop: 11, borderTopWidth: 1, borderBottomColor: colors.line, borderTopColor: colors.line, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, listingWindowLabel: { color: colors.muted, fontSize: 10 }, listingWindowValue: { color: colors.ink, fontSize: 10, fontWeight: '800' },
  expiredHelp: { color: colors.muted, fontSize: 10, lineHeight: 16, marginTop: 11 }, reauditButton: { minHeight: 43, marginTop: 9, borderRadius: 13, flexDirection: 'row', gap: 7, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green }, reauditText: { color: colors.surface, fontSize: 11, fontWeight: '900' }, disabledButton: { opacity: 0.55 },
  revisionHelp: { color: colors.red, fontSize: 10, lineHeight: 16, marginTop: 11 },
  revisionButton: { minHeight: 43, marginTop: 9, borderRadius: 13, flexDirection: 'row', gap: 7, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.green },
  revisionText: { color: colors.surface, fontSize: 11, fontWeight: '900' },
  staleAction: { opacity: 0.55 },
  stockSummary: { marginTop: 12, padding: 12, borderRadius: 13, flexDirection: 'row', justifyContent: 'space-between', backgroundColor: colors.canvas }, stockLabel: { color: colors.muted, fontSize: 9 }, stockValue: { color: colors.ink, fontSize: 11, fontWeight: '900', marginTop: 4 }, manageButton: { minHeight: 42, marginTop: 9, paddingHorizontal: 3, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, manageText: { color: colors.green, fontSize: 11, fontWeight: '900' },
});
